...
function intakeFactFindDocument(intake, note = {}) {
  ...
  const sections = [
    ...
    factSection("Second Applicant Details", [
      ...
      ["Employer", data.secondApplicantEmployerName],
      ["Occupation", data.secondApplicantOccupation],
...
app.get("/api/client-intakes", (request, response) => {
  ...
  const rows = clientIntakes.map((intake) => {
    ...
    return {
      ...
      secondApplicantOccupation: intake.submission?.secondApplicantOccupation || "",
